...
def _check_apt_repo_metadata(
    url: str, dist: str, release_file: Release, errors: RepoErrors, verify: Optional[str] = None
) -> None:
    ...
    if not files:
        errors.add(url, dist, urljoin(dist_url, "Release") + " file malformed")
        success = False
...
def _check_apt_signatures(
    url: str, dist: str, gpg: Optional[gnupg.GPG], errors: RepoErrors, verify: Optional[str] = None
) -> None:
    ...
    success = check_signature(
        url, dist, release_url, gpg, errors, signature_url=releasesig_url, verify=verify
...
def check_apt_repo(
    url: str,
    dists: Optional[Set[str]],
    gpg: Optional[gnupg.GPG],
    errors: RepoErrors,
    verify: Optional[str] = None,
) -> None:
    ...
    if not dists:
        try:
            ...
            errors.add(url, RepoErrors.APT_DIST, error_str)
    ...
    for dist in dists:
        try:
            ...
            try:
                ...
                errors.add(url, dist, f"Could not access Release file at {e.response.url}: {e}")
                continue
            ...
            try:
                ...
                errors.add(url, dist, f"{release_url} file malformed")
                continue
                ...
                for arch in architectures:
                    try:
                        packages = _packages_file(
                            urljoin(dist_url, comp, f"binary-{arch}"), verify=verify
                        )
                    except HTTPError as e:
                        errors.add(
                            url, dist, f"Could not access Packages file at {e.response.url}: {e}"
                        )
                        continue
            ...
            errors.add(url, dist, f"Error when attempting to access {e.response.url}: {e}")
